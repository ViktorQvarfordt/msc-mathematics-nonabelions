...
The Fibonacci anyon model is the one of the simples non-Abelian anyon model containing all the interesting features of non-Abelian anyons. It is commonly studied as a prototypical example of non-Abelian anyons. Another common model is the Ising model, however Ising anyons cannot be used for general quantum computation.
...
\begin{align*}
  ...
  τ \times τ = 1 + τ
\end{align*}
such that $\tau$ is its own anti-particle $\overline{τ}$. We shall refer to the $τ$ anyon as Fibonacci anyons.
...
\begin{corollary}
  ...
  \begin{align*}
    ...
    % \{
    %   R_{ττ}^1 × 3,
    %   \left( R_{ττ}^τ \right)^3 × 2,
    %   \left( R_{ττ}^τ \right)^7 × 3
    % \} \\
    % & =
    % \{
    %   e^{4iπ/5} × 3,
    ...
    % \} \\
      ...
      \right\},
  \end{align*}
  where $a×n$ denotes that $a$ occurs with multiplicity $n$.
...
\chapter{Computing \texorpdfstring{$\rho_n(\sigma_j)$}{ρₙ(σⱼ)} for Fibonacci anyons programmatically}\label{sec:code}
\chaptermark{Computing $\rho_n(\sigma_j)$ programmatically}
